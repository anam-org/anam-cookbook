...
If you're new to VideoSDK agents, start with `realtime_agent.py`. The cascading version is more configurable, but it has more moving pieces. It also shows clearly how pipeline components are sequenced and can be configured independently. The RealTimePipeline uses an "all-in-one" approach and includes a tool-calling example.
