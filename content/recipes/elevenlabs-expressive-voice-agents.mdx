...
<Tip>
A future version of Anam's audio passthrough mode will support passing transcript data inline with the video stream, removing the need for this client-side timing logic. The approach above works today and will continue to work, but keep an eye on the [Anam docs](https://anam.ai/docs/overview) for a simpler path.
</Tip>
