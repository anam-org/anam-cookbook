...
- An ElevenLabs Conversational Agent configured with a V3 voice and `pcm_24000` output format
...
4. Under **Advanced** settings, set the output audio format to `pcm_24000`
5. Copy the **Agent ID**

<tip>
Anam accepts a wide range of sampling rates, but suggests to use pcm_24000 as this provides the best balance of latency and audio quality.
</tip>
<Warning>
Anam's audio passthrough expects raw PCM 16 bit. Other formats (mp3, opus) won't work.
</Warning>
...
The audio input stream tells Anam what format to expect from the incoming audio chunks. This must match the ElevenLabs agent's output format: PCM 16-bit, 24 kHz, mono.
...
- Confirm the ElevenLabs agent output format is `pcm_24000`. Other formats (mp3, opus) are not compatible with Anam's audio passthrough.
