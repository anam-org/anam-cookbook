---
title: Expressive Voice Agents with ElevenLabs and Anam Avatars (Client-Side)
...
<Warning>
**A server-side integration is now available.** Unless you have a specific need for client-side audio bridging (e.g. client tools), we strongly recommend the [server-side ElevenLabs agents recipe](/cookbook/elevenlabs-server-side-agents) instead. It's simpler, lower latency, and provides better session monitoring.
</Warning>
...
The full source code is available on [GitHub](https://github.com/robbie-anam/elevenlabs-agent/tree/clientside_version).
...
- An ElevenLabs Conversational Agent configured with a V3 voice
...
4. Under **Advanced** settings, select an appropriate user input audio format for your application.
5. Copy the **Agent ID**

<Warning>
The user input audio format is processed directly by the ElevenLabs SDK and does not go through Anam's backend. Any sample rate can be set here. 
ElevenLabs audio output format (i.e. ElevenLabs TTS → Anam) is fixed to 16kHz PCM 16-bit mono. Therefore, the agentAudioInputStream needs to be set at pcm_s16le with sample rate 16000 and channels 1. 
</Warning>
...
### Setting up the agent audio input stream

The agent audio input stream declares the format of the TTS chunks ElevenLabs sends to Anam (agent → Anam). 

<Warning>
AgentAudioInputStream is fixed to PCM 16-bit, 16 kHz, mono and is non-configurable in ElevenLabs and is independent of the user input audio format set in ElevenLabs.
</Warning>
...
<Tip>
A future version of Anam's audio passthrough mode will support passing transcript data inline with the video stream, removing the need for this client-side timing logic. The approach above works today and will continue to work, but keep an eye on the [Anam docs](https://anam.ai/docs/introduction/overview) for a simpler path.
</Tip>
...
- Confirm the ElevenLabs agent audio input format is set to `pcm_s16le`, sample rate 16000, channels 1
...
**Avatar drops the first few words of the welcome message**
