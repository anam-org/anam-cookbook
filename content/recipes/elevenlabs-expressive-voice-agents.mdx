...
- An ElevenLabs Conversational Agent configured with a V3 voice and `pcm_16000` user input audio format
...
4. Under **Advanced** settings, set the user input audio format to `pcm_16000`
5. Copy the **Agent ID**

<Warning>
The user input audio format is configurable in ElevenLabs and must be set to `pcm_16000`. Anam's audio passthrough input is fixed at 16 kHz and cannot be changed — any other sample rate will produce garbled or silent audio.
</Warning>
...
The audio input stream declares the format of the chunks Elevenlabs sends to Anam, which currently is non-configurable and fixed to PCM 16-bit, 16 kHz, mono.
