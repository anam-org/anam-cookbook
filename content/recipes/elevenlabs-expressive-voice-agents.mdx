...
<Warning>
**A server-side integration is now available.** Unless you have a specific need for client-side audio bridging (e.g. client tools), we strongly recommend the [server-side ElevenLabs agents recipe](/elevenlabs-server-side-agents) instead. It's simpler, lower latency, and provides better session monitoring.
</Warning>
