...
The agent audio input stream declares the format of the TTS chunks ElevenLabs sends to Anam (agent → Anam). This is fixed to PCM 16-bit, 16 kHz, mono and is non-configurable in ElevenLabs.
It is independent of the user input audio format set in ElevenLabs, which is the format used to ingest the user's microphone into ElevenLabs (user → ElevenLabs).
...
- Confirm the ElevenLabs agent user input audio format is set to `pcm_16000` in the ElevenLabs dashboard. Other input formats are not currently supported with Anam.
