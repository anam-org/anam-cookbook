...
Since `STTTranscriptEvent` does not carry a participant field, we track whose turn is active via `TurnStartedEvent` and only infer scenes from user speech. Without this guard the avatar would switch backgrounds based on the agent's own responses.
...
async def on_turn_started(event: TurnStartedEvent) -> None:
    ...
    if event.participant is None or event.participant.user_id == agent.agent_user.id:
        is_user_turn = False
        return
    is_user_turn = True
...
The `on_turn_started` handler doubles as the neutral-scene reset: when a user turn begins, the background reverts to the base scene, then the transcript handler may switch it to a contextual scene as speech arrives.
...
- It teaches core Vision Agents concepts (`Agent`, `processors`, function tools)
- It shows a practical media customization pattern (chroma-key compositing)
- It stays close to production behavior without turning into a large framework project
