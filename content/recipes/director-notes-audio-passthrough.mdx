...
- Streams it from Cartesia Sonic 3.5 as 16 kHz PCM, with word timestamps
...
Split the typed line at each `[tag]`. Each segment carries the tag (for the face) and clean spoken text. `[laughter]` is kept inline so Cartesia's voice actually laughs.
...
Open one websocket context with `add_timestamps=True` and push each segment's text. Request raw 16 kHz PCM so it drops straight into passthrough with no resampling.
...
<Tip>
Cues can be sent early — the engine latches each onto the persona speech turn that begins within ~1 second, and `at_seconds` sequences them across the turn so each lands on its word. Streaming audio faster than realtime is fine: Anam buffers it and paces playback, and because Cartesia streams faster than realtime each cue reaches the engine before its word renders. `end_sequence()` returns the avatar to a neutral listening pose.
</Tip>
