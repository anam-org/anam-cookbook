...
Add your website origin under **Allowed Domains**, select **+**, and select **Publish**. Add the origin visitors actually use, including any custom domain. For example, `https://example.com` and `https://www.example.com` need separate entries. Origins must match the scheme, hostname, and port; add each origin separately.
