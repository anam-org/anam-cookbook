---
title: ElevenLabs Conversational AI with Anam avatars (Client-Side) (custom TTS example)
tags: [agents, javascript, elevenlabs]
date: 2026-01-19
...
<Warning>
**A server-side integration is now available.** Unless you have a specific need for client-side audio bridging (e.g. client tools), we strongly recommend the [server-side ElevenLabs agents cookbook](/cookbook/elevenlabs-server-side-agents) instead. It's simpler, lower latency, and provides better session monitoring.
</Warning>
<Warning>
**A newer client-side integration is now available.** If you are still looking for a client-side integration, see the [Expressive Voice Agents with ElevenLabs and Anam Avatars (Client-Side)](/cookbook/elevenlabs-expressive-voice-agents) cookbook for a simpler, more efficient approach.
</Warning>
