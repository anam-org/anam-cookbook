---
title: Python audio passthrough with ElevenLabs TTS
tags: [python, audio-passthrough, tts, elevenlabs, beginner]
date: 2025-02-19
...
This recipe shows a script-style example: pass text on the command line, and the script connects to Anam, converts text to speech via ElevenLabs, and streams the audio to the avatar.
...
- Accepts `--text` (ElevenLabs TTS) as input
- Connects to Anam with audio passthrough
- Displays the avatar video in an OpenCV window
...
In order to limit latency, the backend drops incoming audio until the session is established.
...
Anam does not resample the TTS audio internally, so the quality you provide is the quality that will be delibered. Note that for webRTC delivery, the audio is converted to 48000 Hz (stereo) and compressed with OPUS.
</Tip>
...
<Tip>
`end_sequence()` is essential for the avatar to behave naturally at the end of it's turn and return to a neutral listening mode. When end_sequence() is not called, the pipeline assumes TTS audio is still incoming and it will stall the video generation. 
