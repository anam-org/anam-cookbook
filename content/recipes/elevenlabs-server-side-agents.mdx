...
The only difference from a standard Anam session token is the `environment.elevenLabsAgentSettings` field. This tells Anam to connect to the ElevenLabs agent instead of running Anam's built-in STT/LLM/TTS pipeline.
...
Client tools (tools that execute in the browser) are not yet supported. If your agent relies on client tools, use the [client-side integration](/cookbook/elevenlabs-expressive-voice-agents) instead.
