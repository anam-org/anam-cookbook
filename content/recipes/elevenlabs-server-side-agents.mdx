...
Anam's server-side ElevenLabs integration connects Anam directly to an ElevenLabs Conversational AI agent. Instead of bridging audio between two SDKs in the browser, you pass a signed URL when creating an Anam session token and Anam handles the rest.
...
The ElevenLabs connection moves out of the browser and into Anam:
...
The `environment.elevenLabsAgentSettings` field tells Anam to connect to the ElevenLabs agent instead of running Anam's built-in STT/LLM/TTS pipeline. `avatarModel` and `directorNotes` enable a warm Cara 4 performance at the start of the session.
...
No ElevenLabs SDK, no audio bridging, no microphone management. Anam handles the conversation.

### Forwarding client tools

Client tools still execute in the browser, but the ElevenLabs connection can remain server-side. Anam forwards the tool call to the JavaScript SDK, runs the registered handler, then sends the returned result back to ElevenLabs.
